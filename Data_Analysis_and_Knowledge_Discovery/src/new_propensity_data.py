...
def new_yahoo_load():
    ...
    for user in set(train_users):
        ...
        user_mean_pro[user] = np.mean(propensity_for_uesr)
    for i in range(train.shape[0]):
        if train[i, 0] in user_mean_pro:
            train[i, 3] = 0.8 - abs(user_mean_pro[train[i, 0]] - train[i, 3])
    ...
    np.save(file=f'../data/user_mean_pro.npy', arr=user_mean_pro)
