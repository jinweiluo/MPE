...
from trainer_fmf import FmfTrainer
import numpy as np
import argparse
from Bpr import Bpr
from data_bpr import yahoo_load_bpr
...
def main(args):
    if args.model == 'BPR':
        num_ratings, num_user, num_item, train, test, prop = yahoo_load_bpr()
        latent_dim = 30
        learning_rate = 0.005
        regu_lam = 0.0001
        iters = 300
        ...
        bpr = Bpr(train, test, num_user, num_item,
                  latent_dim, Optimizer, learning_rate, regu_lam, prop)
        bpr.build_model(test, iters, batch_size)
        evaluator = Evaluator(test=test, rare=False, name='bpr')
        evaluator.evaluate()
        evaluator_rare = Evaluator(test=test_rare, rare=True, name='bpr')
    ...
    if args.model == 'FMF':
        ...
        evaluator.evaluate()
        evaluator_rare = Evaluator(test=test_rare, rare=True, name='fmf')
        evaluator_rare.evaluate()
    ...
    if args.model == 'RMF':
        ...
        evaluator.evaluate()
        evaluator_rare = Evaluator(test=test_rare, rare=True, name='rmf')
        evaluator_rare.evaluate()
    ...
    if args.model == 'FMF-MPE':
        ...
        evaluator.evaluate()
        evaluator_rare = Evaluator(test=test_rare, rare=True, name='fmf-mpe')
        evaluator_rare.evaluate()
    ...
    if args.model == 'RMF-MPE':
        ...
        evaluator.evaluate()
        evaluator_rare = Evaluator(test=test_rare, rare=True, name='rmf-mpe')
        evaluator_rare.evaluate()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--m', dest='model', default='RMF-MPE')
